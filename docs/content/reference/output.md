...
describes what users get. Pick a format with `-o`, or let vietnamworks choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
vietnamworks <command> -o table   # aligned columns for reading
vietnamworks <command> -o jsonl   # one JSON object per line, for piping
vietnamworks <command> -o json    # a single JSON array
vietnamworks <command> -o csv     # spreadsheet friendly
vietnamworks <command> -o tsv     # tab-separated
vietnamworks <command> -o url     # just the URL column
vietnamworks <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
vietnamworks <command> --fields id,title,url
```
...
```bash
vietnamworks <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
vietnamworks <command>            # a table, because this is a terminal
vietnamworks <command> | wc -l    # JSONL, because this is a pipe
```
